...
class VanillaSlider {
  ...
  constructor(containerId, options = {}) {
    this.webpSupported((isWebpSupported) => {
      ...
      this.bulletsHide =
        typeof this.bulletsHide === "boolean" && this.bullets
          ? this.bulletsHide
          : false;
      this.arrows = typeof this.arrows === "boolean" ? this.arrows : true;
      this.arrowsHide =
        typeof this.arrowsHide === "boolean" && this.arrows
          ? this.arrowsHide
          : true;
      ...
      this.autoPauseOnHover =
        typeof this.autoPauseOnHover === "boolean"
          ? this.autoPauseOnHover
          : true;
      this.webp =
        (typeof this.webp === "boolean" ? this.webp : false) && isWebpSupported;
      this.staticTextPosition =
        typeof this.staticTextPosition === "string"
          ? this.staticTextPosition
          : "SW";
        ...
        [].forEach.call(containerChildren, (containerChild) => {
          ...
          if (containerChild.tagName === "IMG") {
            ...
            if (imageAnchor) {
              ...
              this.images[imagesIndex].linkNewTab =
                imageAnchor.target === "_blank";
            }
            if (containerChild.hasAttribute("text-title")) {
              this.images[imagesIndex].textTitle =
                containerChild.getAttribute("text-title");
            }
            if (containerChild.hasAttribute("text-body")) {
              this.images[imagesIndex].textBody =
                containerChild.getAttribute("text-body");
            }
            if (containerChild.hasAttribute("text-position")) {
              this.images[imagesIndex].textPosition =
                containerChild.getAttribute("text-position");
            }
            if (containerChild.hasAttribute("webp-url")) {
              this.images[imagesIndex].webpUrl =
                containerChild.getAttribute("webp-url");
            }
            ...
            if (containerChild.hasAttribute("title")) {
              this.images[imagesIndex].title =
                containerChild.getAttribute("title");
            }
            imagesIndex++;
          } else {
            console.log(
              "Slider error: invalid container child tag name: " +
                containerChild.tagName
            );
          }
        });
      }
      ...
      if (this.bullets) {
        ...
        this.imageElements.forEach((element, index) => {
          ...
          bullet.style.transition =
            "all " + this.transitionTime / 1000 + "s linear";
        ...
        if (this.bulletsHide) {
          ...
          this.bulletContainer.style.transition =
      ...
      this.resizeContainer = () => {
        this.container.style.width =
          this.container.parentNode.clientWidth + "px";
        var imageXYRatio =
          this.imageElements[0].naturalWidth /
          this.imageElements[0].naturalHeight;
        this.container.style.height =
          parseFloat(this.container.style.width.replace("px", "")) /
            imageXYRatio +
          "px";
      };
      ...
      this.slideFadeOut = (
        ...
        options.fadeTime = options.fadeTime
          ? options.fadeTime
          : defaultFadeTime;
        ...
        if (fadeOutTarget) {
          if (isVisible(fadeOutTarget)) {
            ...
            if (options.directionX) {
              options.directionX = xDirections.includes(options.directionX)
                ? options.directionX
                : null;
              if (options.directionX === "random") {
                options.directionX = ["right", "left", null][
            ...
            if (options.directionY) {
              options.directionY = yDirections.includes(options.directionY)
                ? options.directionY
                : null;
              if (options.directionY === "random") {
                options.directionY = ["up", "down", null][
                  Math.floor(Math.random() * 3)
                ];
              }
            ...
            if (options.waitTime) {
              options.waitTime =
                options.waitTime === true ? defaultWaitTime : options.waitTime;
              options.waitTime =
                typeof options.waitTime === "number"
                  ? options.waitTime
                  : defaultWaitTime;
              setTimeout(() => {
              ...
              options.fadeTime =
                typeof options.fadeTime === "number"
                  ? options.fadeTime
                  : defaultFadeTime;
              ...
              var fadeOutEffect = setInterval(() => {
                if (fadeOutTarget.style.opacity > 0) {
                  ...
                  if (options.directionX) {
                    fadeOutTarget.style.left =
                      parseFloat(fadeOutTarget.style.left.replace("px", "")) +
                      xDirectionInterval +
                      "px";
                  }
                  if (options.directionY) {
                    fadeOutTarget.style.top =
                      parseFloat(fadeOutTarget.style.top.replace("px", "")) +
                      yDirectionInterval +
                      "px";
                  }
                  // zoom a little bit
      ...
      this.goToSlide = (newIndex, callback = null) => {
        if (
          ...
          newIndex < 0 ||
          newIndex + 1 > this.imageElements.length
        ) {
      ...
      this.setSlideText = (index) => {
        ...
        if (this.staticTextBody || this.staticTextTitle) {
          ...
          if (this.images[index].textTitle || this.images[index].textBody) {
            appendTextOverlay(
              this.images[index].textTitle,
              this.images[index].textBody,
              this.images[index].textPosition,
              index
            );
          }
        }
      ...
      this.revealSlideText = () => {
        if (this.textOverlay) {
          var revealEffect = setInterval(() => {
            this.textOverlay.style.opacity =
              parseFloat(this.textOverlay.style.opacity) + parseFloat(0.1);
      ...
      this.transitionSlide = (newIndex, callback) => {
        ...
        this.setSlideText(newIndex);
        this.slideFadeOut(
          this.imageElements[this.currentIndex],
          () => {
      ...
      if (this.swipe) {
        ...
        var handleTouchMove = (evt) => {
          ...
          if (Math.abs(this.swiper.xDiff) > Math.abs(this.swiper.yDiff)) {
            // Most significant.
            var transition = {};
